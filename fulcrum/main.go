...
func (s *FulcrumServer) ProcessVanguardMessage(ctx context.Context, in *pb.FullcrumMessage) (*pb.Notificacion, error) {
...
func (s *FulcrumServer) ApplyCommand(ctx context.Context, comando *pb.PeticionComando) (*pb.CommandResponse, error) {
	fmt.Println("Command received:", comando.Accion, comando.Sector, comando.Base)
	switch comando.Accion {
	case "AgregarBase":
		s.AgregarBase(comando.Sector, comando.Base, int(comando.Value))
	case "ActualizarValor":
		s.ActualizarValor(comando.Sector, comando.Base, int(comando.Value))
	case "RenombrarBase":
		s.RenombrarBase(comando.Sector, comando.Base, comando.NewBase)
	case "BorrarBase":
		s.BorrarBase(comando.Sector, comando.Base)
	default:
		return nil, fmt.Errorf("Accion invalida: %s", comando.Accion)
	}

	vectorClock := make([]int32, len(s.vClocks[comando.Sector]))
	for i, v := range s.vClocks[comando.Sector] {
...
func (s *FulcrumServer) AplicarPropagar(ctx context.Context, p *pb.Propagar) (*pb.PropagationResponse, error) {
	...
	return &pb.PropagationResponse{Success: true, Message: "Propagación realizada correctamente"}, nil
}
...
func (s *FulcrumServer) PropagateChanges() {
	...
	for _, otherServer := range s.otherServers {
		...
		for sector, state := range s.state {
			...
			_, err := fulcrumClient.AplicarPropagar(ctx, message)
			if err != nil {
